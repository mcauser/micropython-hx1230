# Show the Nokia loading image, showing how much bigger this display is

# VCC GND STM32F407ZGT6
from machine import Pin, SPI
spi = SPI(2)
spi.init(baudrate=2000000, polarity=0, phase=0)
cs = Pin('B12', Pin.OUT)
rst = Pin('B11', Pin.OUT)
bl = Pin('B1', Pin.OUT, value=1)  # backlight on

# without framebuffer
import hx1230
lcd = hx1230.HX1230_SPI(spi, cs, rst)

# Nokia bitmap - see nokia_splash.gif
lcd.clear()
lcd.position(0,0)
lcd.data(bytearray(b''))
lcd.position(0,1)
lcd.data(bytearray(b''))
lcd.position(0,2)
lcd.data(bytearray(b''))
lcd.position(0,3)
lcd.data(bytearray(b''))
lcd.position(0,4)
lcd.data(bytearray(b''))
lcd.position(0,5)
lcd.data(bytearray(b''))

# The Nokia display is 84x48
# The HX1230 is 96x68 (12px wider, 20px taller)
# 4032px vs 6528px (62% bigger)

# See nokia_splash.jpg
